...
<main id="main">
  <Router on:conditionsFailed={conditionsFailed} {routes} restoreScrollState={true} />
</main>
<header>
  <nav class="flex items-center border-t divide-x-1 px-1 text-gray-500">
    {#each navData as { path, name, icon }, i}
    {#if i == 0}
      <a href="{path}" use:link class="h-12 px-2 w-full flex items-center justify-center {$location === '/' ? 'text-blue-800 bg-gray-50' : ''}"><i class="gg-{icon}"></i></a>
    {:else}
    <a href="{path}" use:link class="h-12 px-2 w-full flex items-center justify-center {$location.includes(path) ? 'text-blue-800 bg-gray-50' : ''}">
      <i class="gg-{icon}"></i>
    </a>
    {/if}
